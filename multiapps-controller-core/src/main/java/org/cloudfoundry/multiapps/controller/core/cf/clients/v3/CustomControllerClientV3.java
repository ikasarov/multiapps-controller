    ...
    public abstract class ResourcesResponseMapper<T extends CloudEntity> {
        ...
        public void addResources(Map<String, Object> responseMap) {
            ...
            if (included != null && !CollectionUtils.isEmpty(included.keySet())) {
                for (Entry<String, List<Object>> includedEntry : included.entrySet()) {
                    if (includedResources.containsKey(includedEntry.getKey())) {
                        includedResources.get(includedEntry.getKey())
                                         .addAll(includedEntry.getValue());
                    } else {
                        includedResources.put(includedEntry.getKey(), includedEntry.getValue());
                    }
                }
            }
        }
